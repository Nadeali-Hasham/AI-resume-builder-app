...
const RichTextEditor = ({ value = "", onRichTextEditorChange, index = 0 }) => {
  ...
  const handleChange = (e) => {
    const newValue = sanitizeHtml(e.target.value);
  ...
  const generateSummaryFromAI = async () => {
    ...
    setAiLoading(true);
    try {
      const html = await generateExperienceBulletsFromAI(title);
      ...
      toast.error(error?.message || "Failed to generate work experience");
    } finally {
      setAiLoading(false);
    }
  };
